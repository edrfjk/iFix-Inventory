...
<div class="flex flex-wrap items-center gap-3 mb-6">
    <form method="GET" class="flex flex-wrap gap-3 flex-1">
        <select name="type" class="bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">
        ...
        <input type="date" name="from" value="{{ request('from') }}"
            class="bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">
        <input type="date" name="to" value="{{ request('to') }}"
            class="bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search product / ref..."
            class="bg-slate-900 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm w-52">
        <button type="submit" class="bg-slate-700 hover:bg-slate-600 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition">Filter</button>
        <a href="{{ route('transactions.index') }}" class="border border-slate-700 text-slate-400 hover:text-white px-4 py-2.5 rounded-xl text-sm transition">Reset</a>
    </form>
...
<div class="grid grid-cols-3 gap-4 mb-6">
    <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center gap-3">
        ...
        <div>
            <p class="text-white font-bold text-lg">{{ $transactions->total() }}</p>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center gap-3">
        ...
        <div>
            <p class="text-green-400 font-bold text-lg">{{ $stockInTotal }}</p>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center gap-3">
        ...
        <div>
            <p class="text-red-400 font-bold text-lg">{{ $stockOutTotal }}</p>
    ...
    <table class="w-full">
        <thead>
            <tr class="border-b border-slate-800">
                <th class="text-left text-slate-400 text-xs font-semibold uppercase tracking-wider px-6 py-4">Reference</th>
                <th class="text-left text-slate-400 text-xs font-semibold uppercase tracking-wider px-6 py-4">Type</th>
                <th class="text-left text-slate-400 text-xs font-semibold uppercase tracking-wider px-6 py-4">Product</th>
                <th class="text-right text-slate-400 text-xs font-semibold uppercase tracking-wider px-6 py-4">Qty</th>
                <th class="text-right text-slate-400 text-xs font-semibold uppercase tracking-wider px-6 py-4">Unit Price</th>
                <th class="text-right text-slate-400 text-xs font-semibold uppercase tracking-wider px-6 py-4">Total</th>
                <th class="text-left text-slate-400 text-xs font-semibold uppercase tracking-wider px-6 py-4">By</th>
                <th class="text-left text-slate-400 text-xs font-semibold uppercase tracking-wider px-6 py-4">Date</th>
                <th class="px-6 py-4"></th>
            </tr>
        </thead>
        <tbody>
            @forelse($transactions as $txn)
            <tr class="border-b border-slate-800/60 hover:bg-slate-800/25 transition">
                <td class="px-6 py-4">
                    <span class="text-slate-400 text-xs font-mono">{{ $txn->reference_no }}</span>
                </td>
                <td class="px-6 py-4">
                    @if($txn->type === 'stock_in')
                    <span class="inline-flex items-center gap-1.5 bg-green-500/15 text-green-400 text-xs font-semibold px-2.5 py-1.5 rounded-full">
                    ...
                    @else
                    <span class="inline-flex items-center gap-1.5 bg-red-500/15 text-red-400 text-xs font-semibold px-2.5 py-1.5 rounded-full">
                ...
                <td class="px-6 py-4">
                    <p class="text-white text-sm font-medium">{{ $txn->product->name }}</p>
                ...
                <td class="px-6 py-4">
                    <a href="{{ route('transactions.show', $txn) }}"
                        class="text-slate-500 hover:text-blue-400 transition p-1.5 rounded-lg hover:bg-slate-700 inline-flex">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    </a>
                </td>
            </tr>
            @if($txn->remarks)
            <tr class="border-b border-slate-800/30 bg-slate-800/5">
            ...
            <tr>
                <td colspan="9" class="text-center py-20">
                    <div class="inline-flex flex-col items-center">
                        <svg class="w-10 h-10 text-slate-700 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"/></svg>
                        <p class="text-slate-500 text-sm">No transactions found</p>
                        <a href="{{ route('transactions.create') }}" class="text-blue-400 hover:text-blue-300 text-sm mt-2 transition">Record first transaction →</a>
                    </div>
                </td>
            </tr>
            @endforelse
